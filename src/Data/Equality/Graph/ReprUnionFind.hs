...
data ReprUnionFind = RUF (ClassIdMap Repr) RUFSize
    deriving Show
...
unionSets a b (RUF im si) = (a, RUF (IM.update (\case Canonical -> Just $ Represented a; Represented _ -> error "unionSets should be called on canonical ids") b im) si)
